...
export default class Neo4jBackend {
  ...
  async connect(config) {
    this.driver = neo4j.driver(
      config.database || "neo4j://localhost",
      neo4j.auth.basic(config.db.user || "", config.db.password || ""),
      { disableLosslessIntegers: true },
    )
    this.name = config.name || "Neo4j database"
    const opt = config.db.database ? { database: config.db.database } : {}
  ...
  async occurrences({scheme, notation}) {
    const query = `MATCH (t:title)-[]->(n:${scheme.VOC}) WHERE n.notation=$notation RETURN count(t) AS freq`
    return this.readQuery(query, { scheme, notation })
  }

  async coOccurrences({scheme, notation, otherScheme}) {
    // TODO: threshold
    // TODO: test this:
    const query = `MATCH (t:title)-[]->(n:${scheme.VOC}) WHERE n.notation=$notation MATCH (t)-[]->(m:${otherScheme.VOC})
      RETURN [l in LABELS(m) WHERE l<>"concept"][0] AS voc, 
          m.notation AS notation, count(m) AS freq`
    return this.readQuery(query, { scheme, notation, otherScheme })
  }

  async subjects({ppn}) {
    const query = `MATCH (t:title)-[:subject]->(n) WHERE t.ppn=$ppn
      RETURN [l in LABELS(n) WHERE l<>"concept"][0] AS voc, n.notation AS notation`
    return this.readQuery(query, { ppn })
  }

  async records({scheme, notation, limit}) {
    limit = limit > 0 && limit <= 100 ? 1*limit : 10
    const query = `MATCH (t:title)-[:subject]->(n:${scheme}) WHERE n.notation=$notation RETURN t.ppn AS ppn LIMIT ${limit}`
